//
// Created by Islomkhuja Khamidullakhujaev on 2/6/21.
//

#ifndef BOOST_LOG_SERVER_H
#define BOOST_LOG_SERVER_H
...
void Session::writeBuffer(Buffer& t_buffer, const bool once)
{
    ...
    boost::asio::async_write(m_socket,
                             ...
                             {
                                if (!ec && !once) {
                                    std::cout << "ok junatdik endi yana junatamiz" << std::endl;
                                    sendFile(ec);
                                }
                             });
}
