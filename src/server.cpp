...
void Session::handle_read() {
    auto self = shared_from_this();
     ...
     [this, self](boost::system::error_code ec, size_t bytes) {
         if (!ec) {
             std::cout << "handle_read(); o'qib bo'ldi" << std::endl;
             commandRouter(bytes);
         }
...
void Server::doAccept() {
    std::cout << __FUNCTION__ << " client qabul qilishga xarakat qilamiz!" << std::endl;
    m_acceptor.async_accept(m_socket,
        [this](boost::system::error_code ec) {
            if (!ec) {
                std::cout << "async_accept::" <<  " client bizga ulandi!" << std::endl;
